...
function addTodo() {
	...
	const todoObject = generateTodoObject(
		...
		timeStamp,
		false
	); // menyimpan function daripada element-element todo
...
document.addEventListener(RENDER_EVENT, function () {
	...
	for (const todoItem of todos) {
		...
		if (!todoItem.isCompleted) unclompetedTODOList.append(todoElement);
...
function addTaskToCompleted(todoId) {
	// parameternya sebenarnya sama dengan todoObject.id
...
function findTodoIndex(todoId) {
	for (const index in todos) {
		if (todos[index].id === todoId) {
			// jika index dari array todos.id(nya) sama dengan paramater id nya maka...
...
function saveData() {
	if (isStorageExist()) {
		// jika fungsi disamping bernilai true jalankan fungsi dibawah
